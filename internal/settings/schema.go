...
func buildRegistry() []SectionDef {
	return []SectionDef{
		...
		{
			// Forges are their own section because they are their own domain: an
			// entry here contributes no issue tracker, shares no type with one, and
			// is loaded by internal/forge rather than internal/tracker ([SC-3876]).
