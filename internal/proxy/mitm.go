...
func (li *LoggingInterceptor) Intercept(ctx context.Context, conn net.Conn, hostname string, peeked []byte) error {
	...
	if err != nil {
		li.emitOutcome(remoteAddr, hostname, CallRef{}, 0, err, dialStart, nil, nil)
...
func (li *LoggingInterceptor) proxyHTTP(ctx context.Context, client, upstream net.Conn, hostname, remoteAddr string) error {
	...
	for {
		...
		if err != nil {
			...
			li.emitOutcome(remoteAddr, hostname, CallRef{}, 0, err, time.Now(), nil, nil)
		...
		if req.Body != nil {
			...
			_ = req.Body.Close()
			if err != nil {
				li.emitOutcome(remoteAddr, hostname, callRefOf(req), 0, err, start, nil, nil)
		...
		li.emitOutcome(remoteAddr, hostname, callRefOf(req), resp.StatusCode, nil, start, resp.Header, respBodyBuf.Bytes())
