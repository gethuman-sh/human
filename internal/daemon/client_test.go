...
func TestGetDescEditStatus_Success(t *testing.T) {
	...
	st, err := newTestClient(addr, "tok").GetDescEditStatus()
	require.NoError(t, err)
...
func TestGetDescEditStatus_InvalidJSON(t *testing.T) {
	...
	_, err := newTestClient(addr, "tok").GetDescEditStatus()
	require.Error(t, err)
...
func TestDescEditStart_ErrorPropagates(t *testing.T) {
	...
	_, err := newTestClient(addr, "tok").DescEditStart(DescEditStartRequest{Key: "SC-1"})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "daemon command failed")
}
...
func TestDescEditReply_Success(t *testing.T) {
	...
	st, err := newTestClient(addr, "tok").DescEditReply(DescEditReplyRequest{SessionID: "descedit-1", Message: "rewrite it"})
	require.NoError(t, err)
...
func TestDescEditApply_Success(t *testing.T) {
	...
	st, err := newTestClient(addr, "tok").DescEditApply(DescEditApplyRequest{SessionID: "descedit-1"})
	require.NoError(t, err)
