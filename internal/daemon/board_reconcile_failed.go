...
// (the machine already said it will not relaunch); a failure younger than
// FailedRecoveryGrace (the live path's) or older than FailedRecoveryBound (a
// person's). The retry policy itself supplies the rest: the attempt cap, the
// deliberate exits, the outage that goes uncharged.
//
