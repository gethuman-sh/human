...
func TestAdvancePRLoop_reviewApproved_merges(t *testing.T) {
	...
	require.NoError(t, deps.AdvancePRLoop(context.Background(), "SC-1", PRLoopOutcome{ReviewVerdict: PRVerdictApproved, ReviewRecorded: true, FixExit: "", FixPushed: true, FixRecorded: true}))
...
func TestAdvancePRLoop_changesRequested_launchesFixer(t *testing.T) {
	...
	require.NoError(t, deps.AdvancePRLoop(context.Background(), "SC-1", PRLoopOutcome{ReviewVerdict: PRVerdictChanges, ReviewRecorded: true, FixExit: "", FixPushed: true, FixRecorded: true}))
...
func TestAdvancePRLoop_fixDone_launchesReviewer(t *testing.T) {
	...
	require.NoError(t, deps.AdvancePRLoop(context.Background(), "SC-1", PRLoopOutcome{ReviewVerdict: "", ReviewRecorded: true, FixExit: PRFixDone, FixPushed: true, FixRecorded: true}))
...
func TestAdvancePRLoop_fixNeedsInput_escalates(t *testing.T) {
	...
	require.NoError(t, deps.AdvancePRLoop(context.Background(), "SC-1", PRLoopOutcome{ReviewVerdict: "", ReviewRecorded: true, FixExit: ExitNeedsInput, FixPushed: false, FixRecorded: true}))
...
// Convergence guard: a fixer that completed but could not push left the
// reviewed head unchanged, so a re-review would reproduce the same finding
// forever. The loop must red the card with an actionable reason rather than
// spin (SC-1760).
func TestAdvancePRLoop_fixDoneButNotPushed_escalates(t *testing.T) {
	c := &fakeCommenter{comments: []tracker.Comment{
		cmt("[human:ready-for-review]\nbranch: feat/x", time.Unix(1, 0)),
		cmt(prReviewStartedBody("https://example/pr/7", 7, "feat/x"), time.Unix(2, 0)),
		cmt(PRFixStartedHeader, time.Unix(3, 0)),
	...
	require.NoError(t, deps.AdvancePRLoop(context.Background(), "SC-1", PRLoopOutcome{ReviewVerdict: "", ReviewRecorded: true, FixExit: PRFixDone, FixPushed: false, FixRecorded: true}))

	// It is not a decision (no options block) — it is a shipping failure the
	// board must show as a red card so a human pushes the branch.
	_, optionsPosted := posted(c, OptionsHeader)
	assert.False(t, optionsPosted, "an unshipped fix is not a decision")
	failed, ok := posted(c, PRReviewFailedHeader)
	require.True(t, ok, "an unpushed fix must red the card")
	assert.Contains(t, failed, "could not push")
	assert.Zero(t, l.calls, "the guard must launch no re-review")
}
...
func TestAdvancePRLoop_unreadableVerdict_escalates(t *testing.T) {
	...
	require.NoError(t, deps.AdvancePRLoop(context.Background(), "SC-1", PRLoopOutcome{ReviewVerdict: "", ReviewRecorded: true, FixExit: "", FixPushed: false, FixRecorded: true}))
...
func TestAdvancePRLoop_markReadyFails_deployFailed(t *testing.T) {
	...
	err := deps.AdvancePRLoop(context.Background(), "SC-1", PRLoopOutcome{ReviewVerdict: PRVerdictApproved, ReviewRecorded: true, FixExit: "", FixPushed: true, FixRecorded: true})
	require.Error(t, err)
