...
func TestRunBoardFailureWatch_PostsFailedOnIncompleteStage(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		ctx := t.Context()
		go RunBoardFailureWatch(ctx, store, commenterFor, nil, nil, alwaysReachable, nil, nil, nil, StageRetry{}, "", zerolog.Nop())
...
func TestHandleBoardAgentExit_MalformedName(t *testing.T) {
	...
	handleBoardAgentExit(context.Background(), "board-", "", commenterFor, nil, nil, alwaysReachable, nil, nil, nil, StageRetry{}, "", zerolog.Nop())
...
func TestHandleBoardAgentExit_CommenterError(t *testing.T) {
	...
	handleBoardAgentExit(context.Background(), "board-SC-1-planning", "", commenterFor, nil, nil, alwaysReachable, nil, nil, nil, StageRetry{}, "", zerolog.Nop())
}
...
// A PR-loop half-agent's Stop must route into driveLoop with the loop's PM key,
// and must NOT take the generic stage path (which would resolve a commenter and
// mis-parse the compound pr-review suffix via parseAgentName).
func TestHandleBoardAgentExitRoutesLoopAgent(t *testing.T) {
	var loopKey string
	driveLoop := func(_ context.Context, pmKey string) error { loopKey = pmKey; return nil }
	var commenterResolved bool
	commenterFor := func() (tracker.Commenter, error) {
		commenterResolved = true
		return &syncCommenter{}, nil
	}
	handleBoardAgentExit(context.Background(), "board-SC-1-pr-review", "", commenterFor,
		nil, driveLoop, alwaysReachable, nil, nil, nil, StageRetry{}, "", zerolog.Nop())
	assert.Equal(t, "SC-1", loopKey, "the loop agent's exit must drive the loop with its PM key")
	assert.False(t, commenterResolved, "the loop route must not fall through to the generic stage path")
}
...
func TestHandleBoardAgentExit_PhantomCommitFailsLoudly(t *testing.T) {
	...
	handleBoardAgentExit(context.Background(), "board-SC-1-implementation", "", commenterFor, chain, nil, alwaysReachable, commitsPresent, nil, nil, StageRetry{}, "", zerolog.Nop())
...
func TestHandleBoardAgentExit_InContainerReviewCompleteDoesNotChain(t *testing.T) {
	...
	handleBoardAgentExit(context.Background(), "board-SC-1-implementation", "", commenterFor, chain, nil, alwaysReachable, nil, nil, nil, StageRetry{}, "", zerolog.Nop())
...
func TestRunBoardFailureWatch_SkipsChainWhenBranchUnreachable(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		ctx := t.Context()
		go RunBoardFailureWatch(ctx, store, commenterFor, chain, nil, unreachable, nil, nil, nil, StageRetry{}, "", zerolog.Nop())
		time.Sleep(50 * time.Millisecond)
...
func TestHandleBoardAgentExit_UsesDiagnoserHeadlineAndDetail(t *testing.T) {
	...
	handleBoardAgentExit(context.Background(), "board-SC-1-implementation", "", commenterFor, nil, nil, alwaysReachable, nil, diag, nil, StageRetry{}, "", zerolog.Nop())
...
func TestHandleBoardAgentExit_EmptyHeadlineFallsBackToGeneric(t *testing.T) {
	...
	handleBoardAgentExit(context.Background(), "board-SC-1-planning", "", commenterFor, nil, nil, alwaysReachable, nil, diag, nil, StageRetry{}, "", zerolog.Nop())
...
func TestRunBoardFailureWatch_PassesErrorTypeToDiagnoser(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		ctx := t.Context()
		go RunBoardFailureWatch(ctx, store, commenterFor, nil, nil, alwaysReachable, nil, diag, nil, StageRetry{}, "", zerolog.Nop())
		time.Sleep(50 * time.Millisecond)
