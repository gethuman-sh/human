...
- Copies the host's cross-built linux `human` to `/usr/local/bin/human` after creating the container and before starting it, so the agent inside runs the daemon's own build. It is copied, not bind-mounted: Docker Desktop on macOS shares only a fixed list of host directories and turns anything else into an empty directory, while the daemon reads its own install directory unconditionally. A launch with no usable `human-linux-<arch>` fails before the container is created rather than creating one that cannot start.
