...
- A store that just failed is left alone for a doubling interval (30s up to 5m) instead of being re-asked at poll rate, so one unanswered approval dialog stays one dialog; the failure is reported once when it starts and once when it clears, recovery is automatic, and this applies even with `cache_ttl: 0` since no plaintext is involved. A held failure never resurrects a cached value past its maximum lifetime: once the ceiling passes the entry is gone, and the failure surfaces as the failure it is
