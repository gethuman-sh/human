...
3. **Make CI green.** Reproduce the failing checks locally — `make test` scoped to the touched packages, plus `make lint` (not the full `make check`; the deploy CI gate runs the full suite). Fix the failures: a drifted API/signature, a broken call site, a stale test. If a fix changes behavior, pin it with a test.
