...
3. **Run the ONE authoritative full suite** — this is the single full-suite pass of the whole fix. **Detect** the project's full gate (the way `human-done` detects its test runner) rather than assuming a fixed command: probe for a `Makefile` with a `check` (or `test`) target, then common per-ecosystem full gates — `make check`, `make test`, `go test ./...`, `npm test`, `pytest`, `cargo test`, etc. Run the fullest gate the project offers; if no runner is found at all, note it and treat the regression test from step 2 as the available evidence. Capture the exact command and its result verbatim; you will record it as evidence the reviewer trusts instead of re-running the suite.
   - **Green suite** → record it verbatim and continue to step 4.
   - **Red suite** → do NOT immediately return NOT DONE. Go to step 3a to classify each failure. A red suite is only a blocking verdict when a failure is fix-caused; an unrelated, pre-existing, environmental flake must not mask a proven fix.
3a. **Classify a red suite in a clean, isolated worktree** — this matters most when the gate ran in a **dirty or contended shared checkout** (the topology `human`'s own board daemon uses: agents share a bind-mounted host repo, so a real-git or environment-sensitive test can go red for reasons unrelated to the fix). Detect whether you are in that topology before assuming it — signs are an unexpectedly dirty working tree (`git status --porcelain` shows changes you did not make), a worktree shared with other runs, or a dispatch that flags board/daemon context. When the checkout is clean and exclusively yours, a red baseline is more trustworthy and this isolation step is a safeguard rather than a necessity — but running it is always safe. Either way, do not trust a baseline check run inside a contaminated workspace; classify each failure like this:
