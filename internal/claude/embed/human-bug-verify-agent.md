...
3. **Run the ONE authoritative full suite** — this is the single full-suite pass of the whole fix, replacing every redundant full-suite run earlier stages might otherwise repeat: run `make check` (or the project's full gate / `make test` / `go test ./...` / `npm test`). Capture the exact command and its result verbatim; you will record it as evidence the reviewer trusts instead of re-running the suite.
