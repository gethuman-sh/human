...
```
Task(subagent_type="human-ticket-reviewer", model="opus", prompt="Review ticket $ARGUMENTS before it enters the pipeline. Judge whether solving it solves the problem — root or symptom, complete, coherent with existing siblings, right altitude. Act on what you find: reframe, supersede, escalate or reject. Record the verdict as a [human:ticket-review] marker.", run_in_background=false)
```
