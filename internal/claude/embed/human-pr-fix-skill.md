...
The agent reads the machine reviewer's findings from `stage.pr-review` (**and any comment a human left on the PR out of band**), addresses them, commits on the local branch — it does not push; the daemon ships the branch at merge — and records its exit (`done | needs-input | outage`) plus the post-fix head in `stage.pr-fix`. The daemon re-reviews the new commit after a `done`, escalates on `needs-input`, and parks the card uncharged on an `outage`, so you do **not** post board markers or decide the next step yourself: run the agent and report what it changed.
