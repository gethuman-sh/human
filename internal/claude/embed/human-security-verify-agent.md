...
4. **Run the ONE authoritative full suite** — the single full-suite pass of the whole fix. **Detect** the project's full gate the way `human-done` detects its test runner rather than assuming a fixed command: probe for a `Makefile` with a `check` (or `test`) target, then common per-ecosystem full gates — `make check`, `make test`, `go test ./...`, `npm test`, `pytest`, `cargo test`, etc. Run the fullest gate the project offers; if no runner is found at all, note it and treat the security regression test from step 2 as the available evidence. Capture the exact command and result verbatim as evidence the reviewer trusts.
