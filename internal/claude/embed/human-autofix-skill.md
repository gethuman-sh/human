...
{"exit":"needs-human-work",
 ...
 "blocker":{"kind":"exhausted-fix-rounds",
   "evidence":"the bug-verify gaps, verbatim — same text as the marker's evidence field",
...
- **UPHELD** — the verdict stands; act on it:
  - **not-a-bug** — make **no code changes**. Post the terminal marker FIRST: `human marker post <BUG_KEY> no-fix-needed --field verdict=not-a-bug --field challenge=upheld`. Only once it is on the ticket, close it with `human close <BUG_KEY>` (closed-type status, falling back to done-type when the workflow has none). Then Report and STOP.
