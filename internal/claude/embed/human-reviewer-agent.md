...
5. **Trust the recorded verification evidence.** Look for a `[human:bug-verify]` (or `[human:done]`) comment on the ticket whose `## Evidence` block records a green full-suite `result:` on a `commit:` that equals the HEAD Step 0 checked out (`git rev-parse HEAD` on the detached bound branch). When it matches, do NOT re-run the full suite — cite that evidence as the suite result. Re-run ONLY the specific tests your own findings put in doubt (e.g. a file you flag as under-tested). When no such evidence exists, or its `commit:` differs from the reviewed HEAD, run the full suite yourself — the "one green full-suite run before deploy" guarantee must hold. Run that suite (and any targeted re-runs) in a **clean detached worktree of the reviewed commit** (`git worktree add --detach "$wt" <reviewed-sha>`; remove it with `git worktree remove --force "$wt"` when done), never in the contended shared checkout, so an environment-sensitive test does not go red on workspace contamination. Classify any red with the two-tier rule: a failure in the fix's scope OR green on a clean `origin/<base>` baseline worktree is fix-caused and blocks; a failure outside scope AND already red on that clean baseline is unrelated-and-pre-existing — flag it, do not fail the review on it (SC-1135).
...
7. **Flag** missing criteria, unaddressed edge cases, and scope creep beyond the ticket. For this review type, "scope creep" means changes inside the ticket's commits that go beyond the ticket, NOT unrelated commits on the branch (those are simply excluded from the diff and out of scope for this review).
   ...
   **The outcome is the criterion, not the mechanism.** For a bug ticket the acceptance criterion is the stated **outcome** — its `**Expected behaviour**` / "what must become true" — never the remedy the ticket happens to sketch. A `file:line` pointer (often labelled "Observed at" / "as of the scan") and any suggested mechanism in the description are **diagnostic guidance, not criteria**: a stale line number does not invalidate the ticket, and a fix that achieves the outcome by a different, equivalent, or cheaper route is a **PASS**, not a FAIL and not scope creep. Only a mechanism written as an explicit `**Required approach**` constraint (with its stated reason) is binding — evaluate that as a criterion; treat everything else about *how* as guidance.
