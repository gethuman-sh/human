...
- **The plan decides**: a plan step with several valid approaches was settled by the planner, whose contract is that a board plan arrives with every decision made. Take the approach the plan records; where it records none, read the surrounding code and choose the approach that fits it, and name the choice in the handoff notes. Nobody is present to choose an implementation approach for you — a genuine product fork is still the options block in step 7b.
