...
- Resolves tracker roles and topology (`human tracker topology`): which tracker is PM, which is engineering, single vs split — and which entries can open pull requests, since `human tracker list` hides forge entries so no agent picks one as a write target. It also states what a config decided without being asked: a GitHub entry that declared no role is both a tracker and the forge (and is therefore never asked for tickets by unattended passes), and a config carrying no forge at all has nothing for `human pr create` to open a pull request against
...
- Treats a **roleless** `githubs:` entry as forge credentials in the daemon's *unattended* passes — the board listing, the reconcile sweep and the recall sync never ask it for issues. A GitHub ticket listing is a search across every issue the token can see, on its own rate-limited quota; the answer is discarded (only `pm` and `engineering` results are read), so the one thing the request contributed was a 403 rate-limit banner over an otherwise healthy board. Hand-run commands are unchanged — someone typing `human list` or `human index` has said what they want. Declare `role: pm` to keep an unattended listing, or `role: forge` to say plainly what the entry is for
