...
export function isReviewRetryable(card) {
...
export function badgeInfo(card, runningLabels = RUNNING_LABELS) {
    ...
    if (card.state === "running") {
        const text = (card.stage === "done" ? DEPLOY_PHASE_LABELS[card.deployPhase ?? ""] : undefined) ??
            runningLabels[card.stage] ??
            "working…";
        return livenessBadge({ cls: "running", text, title: "Agent running", spinner: true }, card.agentLiveness, `${text} — agent not running`, "No agent is running this stage on this machine — the run died or was stopped. Retry it, or drop the card on its stage again.");
    }
...
export function safetyPollShouldReconcile(_daemonReachable) {
    return true;
}
