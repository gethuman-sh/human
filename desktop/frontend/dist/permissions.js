...
function els() {
    return {
        ...
        approve: document.getElementById("perm-approve"),
        deny: document.getElementById("perm-deny"),
        more: document.getElementById("perm-more"),
...
export function humanizeRequest(r) {
    switch (r.operation) {
        case "DeleteIssue":
            return `Delete ${r.key}?`;
        case "EditIssue":
...
function render() {
    const e = els();
    if (!e.strip || !e.question || !e.meta || !e.more || !e.dropdown)
        return;
    if (queue.length === 0) {
    ...
    if (dropdownOpen && rest > 0) {
        ...
        e.dropdown.innerHTML =
            `<div class="perm-queue-hd">Waiting queue</div>` +
                queue
                    .slice(1)
                    .map((r) => `<div class="perm-row" data-id="${escapeHtml(r.id)}">` +
                    ...
                    `</div>`)
                    .join("");
        e.dropdown.querySelectorAll(".perm-mini").forEach((btn) => {
...
function onKeydown(e) {
    ...
    if (!(e.ctrlKey || e.metaKey) || !e.shiftKey || queue.length === 0)
        return;
