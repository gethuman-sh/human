...
export function initSettingsView(getBindings) {
...
export async function saveSetting(path, value) {
    if (!bindings)
        throw new Error("settings bindings not ready");
...
function renderRow(v) {
    ...
    const readOnlyNote = v.readOnly
        ...
        : "";
    return (`<div class="settings-row" data-row="${escapeHtml(v.path)}">` +
...
function renderEditor(v) {
    ...
    switch (v.type) {
        case "bool":
            return `<input type="checkbox" class="settings-input" data-path="${path}" data-type="${v.type}"${v.value ? " checked" : ""}${dis} />`;
        case "enum": {
            const opts = ["", ...(v.enum ?? [])]
        ...
        case "stringlist":
        case "intlist": {
            const items = Array.isArray(v.value) ? v.value.map(String) : [];
...
function wireSidebar(h) {
    h.querySelectorAll(".settings-sec").forEach((btn) => {
...
function wireRows(h) {
    h.querySelectorAll(".settings-input").forEach((input) => {
        if (input.disabled)
            return;
        const commit = () => void commitRow(input);
...
export function parseInput(type, raw, checked) {
    switch (type) {
        case "bool":
...
async function commitRow(input) {
    ...
    if (input.dataset.writeonly === "1" && input.value === "")
        return;
    let parsed;
    try {
        parsed = parseInput(type, input.value, input.checked);
    }
    catch (err) {
...
function showRowError(path, message) {
