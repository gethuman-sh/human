...
export interface QueueCard {
  ...
  // how long its agent has been silent and against which budget. Absent when
  // the daemon has no record; "stalled" liveness is derived from it by the
  // desktop overlay, never here (SC-5328).
  ...
  // Why a planning card resolved with nothing to plan: "merged", "duplicate",
  // "escalated" or "rejected" — the nothing-to-do record's reason. Absent on a
  // record posted before the reason existed, which renders unlabelled rather
  // than as shipped (SC-5326).
...
export const RESOLVED_REASON_LABELS: Record<string, { text: string; title: string }> = {
...
function livenessBadge(base: BadgeInfo, liveness: string | undefined, deadText: string, deadTitle: string, progress?: AgentProgress): BadgeInfo {
  if (liveness === "stalled") {
