...
let bindings: (() => StatsBindings) | null = null;
let latest: StatsOverview | null = null;
let error = "";
let range: StatsRange = "24h";
...
export function initStatsView(getBindings: () => StatsBindings): void {
...
function stopStatsPollTimer(): void {
  if (timer !== null) {
    clearTimeout(timer);
    timer = null;
  }
...
export function startStatsPoll(): void {
  if (polling) return;
  polling = true;
  scheduleStatsPoll();
}
...
export function rangeCoversHistory(daemonStartedAtISO: string, r: string, now: number): boolean {
  const started = Date.parse(daemonStartedAtISO);
  if (Number.isNaN(started)) return true; // unknown start: don't cry wolf
  const spanMs = rangeSpanMs(r);
  return now - started >= spanMs;
}

function rangeSpanMs(r: string): number {
...
export function contextTokens(r: { input: number; cacheCreate: number; cacheRead: number }): number {
...
function renderHeader(): string {
  const btn = (r: StatsRange, label: string): string =>
    `<button class="stats-range-btn${r === range ? " active" : ""}" data-range="${r}" type="button">${label}</button>`;
  return (
...
function headlineCard(label: string, big: string, sub: string): string {
  return (
...
function renderHeadlines(): string {
  ...
  const cards = [
    headlineCard(
      "Tokens (5h window)",
      fmtUSD(s.tokens.costUSD),
      `${fmtNum(s.tokens.output)} output · ${fmtNum(contextTokens(s.tokens))} context`,
    ),
...
function panelShell(title: string, badge: string, body: string): string {
  ...
  const body = rows
    .map((r, i) => {
      ...
      const seg = (n: number, cls: string): string =>
        n > 0 ? `<span class="stats-seg ${cls}" style="width:${Math.round((n / total) * 100)}%"></span>` : "";
      return (
        `<div class="stats-bar-row">` +
...
function wireRange(h: HTMLElement): void {
  h.querySelectorAll<HTMLButtonElement>(".stats-range-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      const r = btn.dataset.range as StatsRange | undefined;
      if (r) setStatsRange(r);
    });
  });
}
