...
import type { DeployControlView } from "./board-queue.js";

export interface DeployControlOptions {
  // Per-pane class so each control keeps its pixel-stable layout
  // (.deploy-zone for the board, .bug-deploy for the Bugs pane).
  className: string;
  // The bulk-ship action a click runs.
  onClick: () => void;
}
...
export function buildDeployControl(view: DeployControlView, opts: DeployControlOptions): HTMLButtonElement {
