...
export interface ProjectsBindings {
  RecentProjects(): Promise<RecentProject[]>;
  BrowseForProjectDir(): Promise<string>;
  OpenProject(dir: string): Promise<RecentProject>;
}

let bindings: (() => ProjectsBindings) | null = null;
let onOpened: ((project: RecentProject) => void) | null = null;
...
export function initProjectsView(getBindings: () => ProjectsBindings, onProjectOpened: (project: RecentProject) => void): void {
...
export async function showProjectsOverview(initialError?: string): Promise<void> {
  error = initialError || "";
  opening = false;
  const h = host();
...
function render(): void {
    ...
    (recents.length
      ? `<div class="projects-recent-list">` +
        recents
          .map(
            (p, i) =>
              `<button class="projects-recent-item" data-i="${i}" type="button" ${opening ? "disabled" : ""}>` +
  ...
  h.querySelectorAll<HTMLButtonElement>(".projects-recent-item").forEach((btn) => {
  ...
  h.querySelector("#projects-path-open")?.addEventListener("click", () => {
    const input = document.getElementById("projects-path-input") as HTMLInputElement | null;
    if (input?.value.trim()) void openDir(input.value.trim());
  });
...
async function openDir(dir: string | undefined): Promise<void> {
  if (!dir || !bindings || opening) return;
  opening = true;
  error = "";
  render();
  try {
