...
export function descEditSendDefers(state: string): boolean {
...
export function descEditHasUnappliedRewrite(state: string, proposal: string | undefined): boolean {
  return !!proposal && proposal.trim() !== "" && state !== "applied";
}

export interface DescEditStatusLine {
  text: string;
...
export function descEditStatusLine(state: string, error: string | undefined, queued: boolean): DescEditStatusLine {
  if (state === "thinking") {
    return {
  ...
  if (state === "error") return { text: error || "Description chat failed", kind: "error" };
  if (state === "applied") return { text: "Saved.", kind: "info" };
...
export function chatInputHeight(scrollHeight: number, maxPx: number): number {
  if (!(maxPx > 0)) return scrollHeight;
