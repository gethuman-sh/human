...
- Right-click on any card opens a context menu: *Open in tracker* and *Close ticket* (the rare escape hatch for abandoned work — shipped tickets close themselves). Product-backlog cards additionally offer *Create mocks*, which runs the `/human-mockups` skill for the ticket in the project devcontainer; while generation runs the item reads *Creating mocks…*, and once the set exists it becomes *View mocks* and jumps straight to that ticket's **root** set in the Mockups view. The ticket→set link lives in the project's `.human/mockups.json` — never on the tracker.
- The **Mockups view** is a navigable tree, not a single round. From any mockup a host-side *Create variations* control (on the option's toolbar, never inside the mockup) takes free-text change instructions and spawns a child group of variants of just that mockup — the source group is never altered — and variations of variations work identically at any depth. A breadcrumb walks back up to the root while child-group chips navigate down; the root always stays the single entry point *View mocks* opens. A leaf option can be *marked as winner*: the root→leaf path is highlighted as the selected design direction, and picking a different leaf replaces the choice. The winner (and the whole tree's lineage) lives host-side in the manifests and `.human/mockups.json`, so `human mockups chosen <KEY>` hands the chosen mockup to planning and execution agents automatically — no manual copying of design decisions. Abandoned branches can be *pruned* (archived under `mockups/.archive/`, reversible); the root cannot be pruned, and pruning the branch that holds the winner clears the winner.
- The bug rail item opens the **Bugs & Security view**, split into two stacked halves that share the same grid → **Fix** → **Deploy** cycle. Defect and security tickets live here instead of the board's columns.
  - **Bugs** (top half): defect tickets (bug label or bug issue type). A wide grid holds every open bug — five rows tall, cards flowing horizontally — next to a red-bordered **Fix** activity column (the defect counterpart to Code's green). A **+** next to the Bugs header opens a file-a-bug dialog (title + description); the new card appears in the grid immediately with a spinner until the tracker confirms, and the ticket is bug-marked the way the PM tracker natively understands it. A **Findbugs** control sweeps the project for new bugs. Dropping a bug onto Fix launches the autonomous `/human-autofix` pipeline (triage → plan → test-first fix → review handoff; no planning gate, autofix plans itself), the daemon chains the review exactly as it does for builds, and a passing verdict marks the card `fixed ✓`.
  - **Security** (bottom half): security tickets (security label or type), a distinct kind that never overlaps with bugs. A violet-bordered **Fix** column mirrors the bug one. Its **+** files a security issue (carrying the `security` label so every tracker recognises it — there is no universal native security type), and a **Find Security** control sweeps the project for vulnerabilities via the `/human-security` scan (the counterpart to Bugs' Findbugs), filing each surviving finding as a security ticket. Dropping one onto Fix launches the `/human-security-fix` pipeline: the same shape as autofix, but triage builds a **threat model** and rates severity, verify's bar is "the attack no longer works", and the review adds a security lens.
  - A single **Deploy** button spans both halves at full height on the right. It is kind-agnostic: dropping a fixed bug or vulnerability ships that one, and a click ships every fixed bug and vulnerability at once — through the same push → PR → CI gate → merge pipeline as the board's Deploy drop. (Which pipeline *fixed* the card is decided at the Fix drop, per half; deploy is the same for both.)
...
- Instant open (stale-while-revalidate): a cold open — including after an app restart — paints the last-known cached board snapshot immediately instead of blanking on a spinner while the live tracker fetch runs, then a background fetch silently swaps in fresh tickets when it lands, with no visible staleness cue or flicker. The snapshot is persisted per project at `~/.human/boardcache.json` (`internal/boardcache`) after every successful full fetch; a cache miss (first run ever, a different project, or a corrupt file) falls back to the original spinner + quick-titles path.
- Hand-sorted card order within a queue column and parked (hidden) cards are local view preferences persisted per project at `~/.human/boardprefs.json` (`internal/boardprefs`) — never a label, comment, or status on the ticket — so hiding or reordering a card in one project does not affect another.
- Live updates: subscribes to the daemon and refetches board cards on every change event. A small independent poll (every 3s) drives only the header daemon-reachability dot, since there is no daemon-pushed event to subscribe to when the daemon itself is down — periodic ticks layered on top of the daemon subscribe channel.
...
- The gear rail item opens a full settings view over `.humanconfig.yaml`: a section sidebar (project, appearance, trackers, knowledge, messaging, vault, daemon — sections appear as configured) with per-field forms. Edits save on blur (✓ flash on success, inline error on rejection) through the daemon's `config-set` route, which rewrites the file with a comment- and order-preserving YAML round-trip — the file stays hand-editable and git-diffable.
...
- Board appearance lives in the config like every other setting, not in the stylesheet. **Settings → Appearance → Not-mine card dimming** (`ui.dim_percent` in `.humanconfig`) is how faint a card owned by someone else renders, as an integer percent of full opacity: 5–100, default 35, and 100 turns the dimming off for a solo user. It is applied as `--not-mine-opacity` on the document root at render time, with the `:root` value in `frontend/static/style.css` as the shipped fallback — so every rule derived from that one token (hover return, the degraded/not-mine overlap) follows automatically, an unconfigured project renders exactly as before, and a value outside the range falls back to the default rather than hiding a column. Changing it and refreshing the board shows the new dimming: no rebuild, no daemon restart. Who counts as "me" is the other half of the same feature and is declared separately under `me:`.
...
- Closing the app window always stops the daemon it manages — the same provenance-agnostic stop Switch Project uses, so an adopted daemon is stopped exactly like one the app launched. An idle daemon (no working instance, no live ticket-stage lease) is stopped silently and the window closes immediately; a busy one shows a three-way dialog (Cancel / Stop anyway / Wait and close) before anything happens. If the app never got to run its close flow (crash, force-quit, OS shutdown), the daemon is left running; the next launch detects that — a daemon whose recorded managing app process is gone — and offers to clean it up, without nagging about a daemon a user deliberately runs standalone.
