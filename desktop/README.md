...
- Each column carries a muted caption under its header explaining what its inbound transition runs — the ideation conversation, the `human-plan` comment, the `human-execute` branch/commits/auto-review cycle, or how cards earn Ready to Deploy; the Deploy zone explains itself in a tooltip.
