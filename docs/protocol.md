...
- **`MinProtocol`** — the oldest client this daemon still serves. Raised
  **only** for breaking changes. This is the conscious compatibility decision:
  ...
  which clients are cut off. A daemon at protocol 11 with MinProtocol 8 keeps
...
| 10 | 2026-09-25 | SC-3656 board card names the review→fix round and bounds the ended-card phase read to the run that reached it. Added: `BoardCard.PRReviewRound`/`PRReviewRoundCap` (`pr_review_round`/`pr_review_round_cap`) and `BoardViewCard.PRReviewRound`/`PRReviewRoundCap` (`prReviewRound`/`prReviewRoundCap`), the loop round a running done-stage card is in and the `DefaultPRReviewRounds` bound it runs against; and `BoardCard.StageRunStartedAt` (`stage_run_started_at`) / `BoardViewCard.StageRunStartedAt` (`stageRunStartedAt`), the Created time of the current stage run's "-started" marker. `MinProtocol` stays at 3 — purely additive, as row 6. `MinDaemonProtocol` stays at 8 — an older daemon simply omits all three fields: the round badge falls back to today's "PR review…"/"fixing PR findings…" rendering with no round, and an ended card with no `StageRunStartedAt` shows no phase at all rather than reading one unbounded — neither misbehaves. | 3 | 8 |
| 11 | 2026-09-26 | SC-6016 the launch briefing on request. Added: the `feedback` route (`internal/daemon/feedback_route.go`), `FeedbackRequest`/`FeedbackReport` and `Client.Feedback`, reached from the new `human feedback <KEY> <STAGE>` command; the route answers from the daemon's own findings record, model runner and per-project launch cache, so what it prints is what a launch of that stage appends (SC-5959). `MinProtocol` stays at 3 — additive, as rows 7 and 8. `MinDaemonProtocol` moves to 11 for their reason: the route is called unconditionally once the command is invoked, and on a daemon at 10 it would die with cobra's "unknown command \"feedback\"" instead of the rebuild-the-daemon error. | 3 | 11 |
| 12 | 2026-09-26 | SC-5878 board badge for a deploy queued behind the ticket's own implementation checkout. Added: `BoardCard.DeployQueueAbandoned` (`deploy_queue_abandoned`) / `BoardViewCard.DeployQueueAbandoned` (`deployQueueAbandoned`), the one-line reason a queued deploy was given up while that withdrawal is still the ticket's newest record; and widened `deployPhase`'s value set with `"deploy-queued"` (`DeployPhase`/`deployPhase`, already-existing string fields, no new field). `MinProtocol` stays at 3 — purely additive, as rows 6 and 10. `MinDaemonProtocol` stays at 11 (row 11's value) — an older daemon simply omits `deployQueueAbandoned` and never emits `"deploy-queued"` as a `deployPhase` value, so the badge falls back to today's rendering rather than misbehaving. | 3 | 11 |
