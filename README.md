...
| `/human-mockups` | Create annotated static HTML mockups exploring N distinct UI options for a feature, matched to the project's real look; iterate any option into variation groups (`--variation`) and mark a winner the board hands to planning/execution |
...
Only a `confirmed` bug that passes the verification gate moves on. To run the quality suite exactly once per fix (SC-782), the tiers are split: the fixer iterates on the **fast test+lint tier** to go green, and the **verify gate runs the one authoritative full-suite pass** — regression test fails before the fix, passes after, the full suite green — recording that result as a commit-bound `## Evidence` block (branch/commit/command/result) on the ticket. The fix lands on an `autofix/` branch with commits referencing the ticket trail (both the PM and engineering keys in split topology, the single bug key otherwise), and a `[human:ready-for-review]` comment is posted on the PM ticket carrying the `branch:` and `commits:` lines — plus an `engineering:` line in split topology; when that line is absent, reviewers review the PM ticket itself.

The run then chains into the review, exactly like the kanban flow chains a clean build: the **human-reviewer** agent reviews the branch against the plan and acceptance criteria, **trusts the recorded full-suite Evidence when it is bound to the reviewed commit** (re-running only the tests its own findings put in doubt, instead of repeating the whole suite), and posts the verdict as a `[human:review-complete]` comment. A `pass` (or `pass with notes`) drives the same deploy pipeline as the board's Deploy stage — PR opened, CI gate, merge, branch deleted, ticket moved to done, `[human:deployed]` posted. A `fail` gets one rework cycle (fixer → verify → re-review); if it still fails, the run stops honestly with the handoff standing for a human and no PR merged. When autofix runs *as a board stage agent* (dropped on the Bugs pane's Fix column), the review runs **inline in the same warm implementation container** (SC-782) — one container startup, not two — so the board shows one continuous run instead of two sequential containers; only the deploy is left to the Bugs pane's Deploy button, which ships the reviewed fix. CI remains the final gate on every merge.
